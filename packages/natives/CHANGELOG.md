...
- Added `TaskRun.notePhaseTokens(owner, tokens)` and the `phase_tokens` trace record, so a run's cost is charged per attempt rather than per passed phase — a rejected attempt spends real tokens. Its own record because `value` already carries the violation count on both rejection paths. The record's `ok` flag means *accounted for*: a zero is a provider that reported no usage, not a free turn, and a reader must be able to tell those apart.
- Added `TaskRun.noteGateReport(gate, checks)` and `taskEnvelopeText(turn)`: an escape hatch for checks the engine cannot perform. A caller-run gate is merged with the native ones on the next submission, traced as `gate_check`, and drained per attempt so a stale report never judges a corrected turn. `taskEnvelopeText` exposes the engine's own "last top-level JSON object" rule so a caller inspecting the payload does not reimplement it and drift.
- Added the `json_parses` gate to `taskGateNames()`: every declared `.json` artifact must parse. `files_non_empty` counts bytes, including the bytes of an apology, so structure needed its own check. Zero new dependencies — `serde_json` was already there.
- Added the `diff_matches_claims` gate to `taskGateNames()`: it compares the commit the run started on plus `git status` (untracked files listed individually) against the paths phases declared, so a change nobody confessed fails the phase. Implemented here rather than in `pi-tasks` because it needs git, keeping the engine crate free of I/O beyond the filesystem.
- Added `undeclaredIgnore` on `TaskRunOptions`: globs `diff_matches_claims` treats as accounted for. Compiled once at run construction so a malformed pattern is reported with the pattern itself before any phase runs, and empty by default so the gate forgives nothing it was not told to.
- **Breaking:** `artifacts_exist` and `files_non_empty` now fail an envelope with no artifacts instead of passing vacuously. Requesting either gate asserts that the phase produces files, and a phase must not clear a gate by claiming nothing.
