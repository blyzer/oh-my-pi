...
export async function runWorkflow(request: WorkflowRequest): Promise<WorkflowResult> {
	...
	if (result.status === "accepted") {
		await appendEvent(runDir, workflowId, "WorkflowAccepted", { phase });
	} else {
		await appendEvent(runDir, workflowId, "PhaseFailed", { phase });
		await appendEvent(runDir, workflowId, "WorkflowFailed", { phase });
	}
	return result;
}
