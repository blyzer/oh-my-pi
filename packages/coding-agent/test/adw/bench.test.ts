 ...
 *
 * The registry's value is not that it runs scenarios — Bun already does that
 * — but that each is named, addressable, and grouped by the safety family it
 * defends, so an uncovered family is visible instead of merely absent.
 */
import { afterEach, describe, expect, it } from "bun:test";
import * as fs from "node:fs/promises";
import * as os from "node:os";
import * as path from "node:path";
import { BENCH_FAMILIES, scenarios, uncoveredFamilies } from "../../src/adw/bench";
import "../../src/adw/scenarios";
...
describe("FactoryBench", () => {
	for (const scenario of scenarios()) {
		it(`${scenario.id} — ${scenario.asserts}`, async () => {
			...
			// should say what it saw, not merely that it was false.
	...
	it("reports which safety families are unproven", () => {
		...
		// Not an assertion that everything is covered — it is not. This fails
		// only if the registry stops being able to answer the question, which
		// is the one thing a benchmark must never lose.
	...
	it("keeps the canonical fail-closed scenario registered and held out", () => {
		// FB-FAILCLOSED-001 is the case the engine must never optimize itself
		// into passing incorrectly. Losing its `canonical` mark would make it
		// an ordinary scenario, eligible for tuning like any other.
