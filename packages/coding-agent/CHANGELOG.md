...
- `/adw` runs now record what each attempt cost. Charged per attempt and before the verdict, because a rejected attempt spends real tokens: in a measured two-attempt run the rejected try cost 25,561 tokens against the accepted one's 26,059, so counting only successes understated the run by half. The trace viewer shows per-phase and per-run totals.
