...
- Added `schema` on `/adw` `agent` and `fusion` phases: a JSON Schema the envelope payload must satisfy, reported in the trace as the `payload_matches_schema` gate. Violations name the field and arrive all at once, because "expected boolean" alone makes the agent guess which one and one field per attempt burns the budget. Structure is enforced, not semantics — conditionals (`if`/`then`, `allOf`, `oneOf`, `not`) are **rejected at load time**, because omptype compiles them and then ignores them: a schema declaring "approved implies nothing blocking" validated `{approved: true, blocking: ["x"]}` as fine. A conditional that silently does nothing reads like a guarantee, so a cross-field rule belongs in a `code` phase where an exit code cannot lie.
- Gates the engine cannot run itself are now reported back through `TaskRun.noteGateReport` — the same bargain `code` phases make, where the caller executes and the engine judges. Schema validation needs omptype, and a JSON Schema validator inside `pi-tasks` would have cost that crate its three dependencies and its ability to be tested without a JavaScript runtime. Caller-run results are traced as `gate_check` and block acceptance exactly like native gates, are drained per attempt so a stale report never fails a corrected one, and locate the envelope with the engine's own `taskEnvelopeText` rather than a second implementation that would drift.
