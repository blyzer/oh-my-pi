...
- A process's daemon presence record can now carry what it is doing, not only that it exists. `registerDaemonProjectPresence` wrote `{pid, id, projectDir}` once and never again, and returned a handle whose only member was `close()` — the call site in `main.ts` did not even bind it, because the sole consumer was the postmortem cleanup registered inside the factory. The record gains optional `sessionId`, `sessionFile`, `collabSocket`, `generation` and `startedAt`, the handle gains `update()`, and the call site binds it. Every new field is optional on purpose: an older omp keeps writing the three-field shape, and a reader that treated the additions as mandatory would decide those sessions do not exist. `update()` reports failure rather than throwing, and callers should retry rather than latch off — the failures that reach it (EACCES/EMFILE while the runtime dir is recreated, a concurrent sweep) are transient, and giving up permanently drops a live session out of the directory for the rest of the process lifetime. Writes go through a temp file and a rename, because the record now changes after registration and a torn read looks exactly like a malformed record to the existing sweep, which deletes those.
- Added `findLiveCollabSessions(runtimeDir)`: the live omp processes in a project scope that are offering a local collab room, filtered by a `kill(pid, 0)` probe so a record that outlived its process is not a candidate. It is deliberately read-only — pruning already belongs to `hasLiveDaemonProjectPresence`, and a reader that deletes can delete a record another process is halfway through rewriting. `CollabHost.startLocal` publishes its socket path through the presence handle once the transport is listening and the taps are installed, and retracts it in teardown before dropping those taps, since a record advertising a socket that no longer accepts connections is worse than no record at all.
...
- Added the `diff_matches_claims` gate: every path the working tree changed must be one some phase declared. The existing gates catch a claim with no file; this catches a file with no claim, which is the failure that matters — an agent that edits three files and confesses one leaves two changes nobody reviewed, and an existence check cannot see them. The allowed set is cumulative (dirty before the run started, plus every earlier phase's declared paths), so the operator's own uncommitted work is never blamed on a phase. Untracked files are listed individually, renames report both paths, and a gate that cannot read a repository fails instead of passing quietly.
- `/adw` runs now record what each attempt cost. Charged per attempt and before the verdict, because a rejected attempt spends real tokens: in a measured two-attempt run the rejected try cost about as much as the accepted one, so counting only successes understated it by half. The trace viewer shows per-phase and per-run totals.
- Added `onFail: correct` on `code` phases: a failure returns to the nearest preceding `agent` or `fusion` phase as a correction in that agent's own session, instead of re-running the command. A retry could not fix a deterministic step — a red test suite re-run is red again, so it burned the whole budget while the agent that wrote the code never learned it broke. The attempt is charged to the target, which is the only phase that can change the outcome and what makes the loop finite; a target that has exhausted `maxAttempts` halts the run like any other. `onFail: correct` with no correctable predecessor fails the file at load time rather than mid-run. Phase records are now a history, so a phase that ran twice appears twice with the failure that sent the run back in between.
- Added the workflow-level `undeclaredIgnore` glob list, for paths a build legitimately rewrites without any phase claiming them (`bun.lock`, generated sources). Without it `diff_matches_claims` was unusable in any repo where a step runs an install. The default is empty on purpose: a wide default makes the gate noisy, and a gate whose forgiveness an operator cannot audit gets deleted. A malformed pattern fails the run at construction, naming itself, rather than when the gate first fires and a phase has already paid.
- `/adw` cost accounting now charges `usage.totalTokens` rather than the subagent's `tokens` counter, which deliberately excludes cache reads — right for a cumulative billing-volume number, wrong for what one attempt cost. A charge whose provider reported no usage at all (`omniroute/auto` returns an all-zero usage record) is marked unaccounted instead of being summed as zero: a model turn cannot genuinely be free, so the viewer says `provider reported no usage` and a run with any such phase shows the count beside its total. The total is a floor, not a confident sum over numbers nobody reported.
- `diff_matches_claims` now compares against the commit the run started on, not only the working tree. A phase that committed its own changes hid them: `git status` went clean and the gate reported that every change was declared. Measured — a `code` phase running `git add -A && git commit` passed a gate that rejected the identical command without the commit. Untracked files still come from status, because they never appear in a diff; neither source alone sees every change.
- Added three runnable `/adw` example workflows under `docs/adw/examples/`, each teaching one thing: `fix.yml` the correction loop, `ship.yml` isolation plus undeclared-change detection, `review.yml` a fusion panel. `/adw` shipped with none, so the first thing a new user had to do was write YAML against a schema they had not read. A test parses every file in that directory, so a schema change that invalidates an example fails in CI rather than in someone's first run.
- Added `dependsOn` on `/adw` phases: execution order follows the declared graph instead of the order phases happen to appear, so an author declares what needs what rather than hand-sorting the list. The graph is declared and never inferred — a graph a model proposes changes between runs given the same prompt, and then `resume` cannot rebuild a position in a plan that no longer exists. The sort is a pure function of the file: ties break on declaration order, so two runs execute in the same sequence and a resumed run derives the order it originally ran. A cycle, a self-dependency, or an unknown name fails the file at load time and names every phase in the loop; the engine leaves an unorderable graph in declaration order rather than reordering it into something nobody wrote. `onFail: correct` now follows the graph too, sending a failure to the nearest dependency that has an agent to correct rather than to whatever was declared above it.
- Added per-seat `prompt` on `/adw` fusion panels, so a panel can divide work instead of only asking the same question twice: N read-only seats investigate N things concurrently and one writer merges them. A seat with its own part is told to answer only that part rather than that it is "one opinion of N". This is safe because no panel seat may write — parallel writers remain out, since two agents writing the same tree lose writes and `diff_matches_claims` reads the result rather than the order.
- Fixed the fuser's own `prompt` being accepted by the schema and silently dropped. A live run wrote no file because the only instruction naming the output was never delivered; an accepted key that does nothing is worse than a rejected one.
- **`artifacts_exist` and `files_non_empty` now fail an envelope that declares nothing**, instead of passing vacuously. A fusion phase returned `artifacts: []`, cleared both gates and wrote no file — while the docs claimed a phase could not pass by claiming nothing. Requesting either gate is an assertion that the phase produces files. A `code` phase declares no artifacts by construction, so either gate on one is now rejected at load time rather than failing mid-run, and the message says to use `diff_matches_claims` or move the gate to the agent phase that writes the files.
- Added the `json_parses` gate: every declared `.json` artifact must actually parse. Bytes are not structure — a phase that hands the next one a truncated `plan.json`, or one holding an apology instead of an object, satisfies `files_non_empty` because it counted the bytes in the apology. Scoped by extension, so non-JSON artifacts are left alone, and the violation carries the parse position because "it is invalid" is not actionable. Like the other claim-scoped gates it belongs on `agent` and `fusion` phases and is rejected on a `code` phase at load time.
- Added `schema` checks on `/adw` `agent` phases, reported as the `payload_matches_schema` gate with all validation errors. Validation uses omptype’s JSON Schema subset: self-contained `anyOf`/`allOf` branches can enforce cross-field rules, while known unsupported constructs such as `if`/`then`/`else`, `oneOf`, and `not` are rejected at load time. Corrected the blanket rejection of compositions and false rejection of property names or annotation data containing keyword names. The importer still drops sibling constraints beside compositions and numeric/string bounds without an explicit type; the ADW guide documents these limits and a working approval/blockers schema. Native `.narrow()` predicates are a separate JavaScript API, not preserved by JSON Schema export.
- Gates the engine cannot run itself are now reported back through `TaskRun.noteGateReport` — the same bargain `code` phases make, where the caller executes and the engine judges. The JavaScript caller uses omptype; this integration adds no JSON Schema validator to `pi-tasks`. Caller-run results are traced as `gate_check` and block acceptance exactly like native gates, are drained per attempt so a stale report never fails a corrected one, and locate the envelope with the engine's own `taskEnvelopeText` rather than a second implementation that would drift.
- ADW now validates custom output contracts on both agent and fusion-fuser envelopes and includes the actual schema in writer prompts. Added the native-omptype `verdict_consistent` caller gate to reject contradictory or unsupported review verdicts. Opt-in `acceptance: review` separates a valid report from an approved delivery: a coherent rejection passes the review phase but refuses the run and isolated patch application; only a coherent approval in the last accepted envelope can authorize delivery. The same decision is reconstructed on resume and recorded in the terminal trace.
- Added `onReject: { to, maxRevisions }` on `/adw` review phases: a coherent `approved: false` verdict now revises the named earlier builder instead of only refusing delivery, carrying the full rejected report — blockers, unmet requirements, evidence — into the builder's own session, then re-running every phase from the builder through the review. The engine distinguishes three outcomes rather than the prompt: a malformed or contradictory review corrects the *reviewer* within `maxAttempts`; a coherent rejection with budget left revises the *builder*; a coherent approval continues. Budgets are separate and cumulative — each consumed revision grants the target exactly one additional attempt, never a fresh allocation, so `maxAttempts: 1` with `maxRevisions: 2` permits the initial build plus two review-requested rebuilds and nothing more; exhaustion halts with the review's own reason as a `review_exhausted` record. Revision invalidates the superseded builder/check/review records (`invalidated` in the summary, struck through in the CLI and viewer) so no green result from before the edit can authorize delivery, and a revisited fusion review re-polls its panel while a fuser format retry still reuses it. Routes are validated at load time — unknown, self, `code`, later-executing or non-dependency targets and non-integer revision budgets fail the file — and the binary trace bumped to format v4 (36-byte records, u32 attempt, `review_revision`/`review_exhausted` kinds) so resume rebuilds grants, invalidation and the pending correction exactly; older traces are refused by path instead of half-decoded.
- Added `inputs: [phase, …]` on `/adw` phases: a phase now declares which predecessor outputs it consumes instead of inheriting whatever the previous phase happened to leave — a `plan → check → build` pipeline used to hand `build` the check's exit summary because a passing `code` phase replaced the handoff. Accepted envelopes are persisted versioned (`envelopes/<phase>.<version>.json`, the phase's acceptance ordinal) so a correction loop re-running a producer gets a new version instead of overwriting evidence a consumer already read. Selection happens at dispatch and is recorded as one `input_selected` trace event per input (trace format v5; the layout is unchanged and older versions are still refused by path), so the viewer and a resumed run can state exactly which producer versions authorized an execution. Writers receive one labelled section per producer; a `code` phase receives a JSON file via `$ADW_INPUTS` keyed by producer with version, summary, artifacts, notes and payload. A join names both predecessors and gets both regardless of execution order; after a revision the consumer's next dispatch selects the producer's new version. A producer whose current output is missing, invalidated without re-acceptance, or unreadable on resume fails the run naming consumer, producer and version rather than silently substituting the last envelope. Load-time validation rejects unknown, duplicate, self, later-executing, or non-dependency inputs.
- Added `writes` on `/adw` writer phases and workflow-level `protected`: gates verify what a phase *claimed*, scopes bound what it may *touch*. When any scope is declared, every writer attempt runs between a content snapshot and a settlement — on success, malformed output, provider failure, timeout, cancellation or crash — and every change since the snapshot is classified by content hash, so a same-size rewrite cannot slip through. A change outside the phase's `writes`, or touching `protected` or the always-protected `.omp/adw/**` (protection beats authorization: declaring the forbidden path does not authorize it), is restored byte-for-byte and the attempt is rejected with a `write_scope` report naming each path; the writer's authorized work and the operator's untouched dirt survive settlement exactly. The run baseline is captured once and persisted, so resume does not relabel interrupted work as user dirt, and `diff_matches_claims` now commits claims to the shared accepted set only when the engine accepts the attempt — a rejected attempt's confession stops expanding later permission. Rollback that cannot be trusted fails closed: the unauthorized diff is preserved as a patch and the run halts instead of retrying. Writers are told their scope in their prompt.
- Added `docs/adw/examples/sdlc.yml`, the complete factory recipe composing every `/adw` mechanism: `plan → build → docs → verify → review`, with explicit `inputs`, per-writer `writes` scopes, a protected evaluator, `onFail: correct` for red checks, `onReject` for coherent reviewer rejections, and `acceptance: review` + `isolation: true` as the delivery contract — the accepted diff reaches the checkout only on approval, a rejected or exhausted run leaves it byte-for-byte untouched with the work preserved as a patch, and nothing is ever staged, committed or pushed by the pipeline. The shipped `verify` command deliberately exits 1 naming itself: adaptation to the target repository's real test/lint/typecheck entry points is the installation, because a placeholder that exits 0 would deliver unverified code. The guide documents the composition, the delivery contract and the adaptation path.
- `/adw` recovery is now durable across process death, not just clean failures. The run directory holds the full contract — `workflow.json` (resume under an edited definition is refused naming the record), `claims.json` (accepted `diff_matches_claims` authorization loaded on resume instead of recapturing the tree, so a crashed attempt's undeclared edits are never relabelled as operator dirt), the write-guard baseline, `isolation.json` and `delivery.json`. In-place rejections now persist their full correction as a `correction_pending` trace record (format v6, layout unchanged, older versions refused), so a resumed retry knows *what* was wrong instead of repeating the attempt blind; a `maxAttempts` differing from the traced base refuses naming both numbers. Isolated runs resume by **reattaching** the surviving sandbox — never rebuilding it from the base, which would silently redo completed phases — and delivery happens exactly once: the settlement outcome is recorded before anything else can touch the tree, an accepted run that died inside the delivery window is recovered by settling from the record, and a settled run refuses to resume rather than risk applying its patch twice. The viewer now reports the LAST terminal record as the run's current state, keeping a crash's failed terminal in the timeline as history instead of presenting it as the verdict.
...
- An accepted phase that does not itself carry `diff_matches_claims` now still commits its declared artifacts to the run's shared claims set. A live sdlc run showed `plan` (artifact gates only) writing PLAN.md and `build` — the first phase carrying the diff gate — being blamed for it, because claims were only committed through the gate's own accept hook. Acceptance is what authorizes, not which gate a phase happens to carry; rejected attempts still commit nothing.
- Fixed the shipped `ship.yml` example naming the read-only `scout` agent as the owner of a phase whose gates require writing PLAN.md — the phase could only ever burn its attempts. Both it and the new recipe use a writer-capable owner for planning.
...
Older entries are archived in [packages/coding-agent/CHANGELOG.md@d11314efc273](https://github.com/can1357/oh-my-pi/blob/d11314efc273b81e00b8a146bb82033e2442d2b7/packages/coding-agent/CHANGELOG.md).
## [17.1.7] - 2026-07-27
