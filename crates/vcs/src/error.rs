...
//! empty cherry-pick) are first-class variants here so callers can match on a
//! structured [`Error::kind`] instead of message text.
...
/// Format a CLI failure: prefer stderr, then stdout, then a generic
/// exit-code message.
