...
pub mod matchers {
	...
	mod entry {
		...
		pub struct WalkEntry {
			...
			/// Operand-relative path used for display and path-based matching,
			/// when it differs from the real filesystem path. The shell host
			/// roots the walk at a working-directory-resolved (often absolute)
			/// path so stat/exec/delete target the correct files even though
			/// the process cwd differs from the shell cwd; this preserves the
			/// operand-prefixed path GNU find prints and matches against (e.g.
			/// `find .` -> `./a`). `None` falls back to `path()`.
		...
		impl WalkEntry {
			...
			/// (`-path`, `-regex`, `-printf %p/%h/%P/%H`). Falls back to
			/// [`Self::path`] when no display override was installed (explicit
			/// entries, unit tests).
			...
			/// starting-point `operand` and the `resolved_root` the walk was
			/// rooted at. The real filesystem path is left untouched. When this
			/// entry's path is not under `resolved_root` (e.g. a followed
			/// symlink escaping the root) the override is left unset and
			/// display falls back to the real path.
			...
			/// appropriate. Multiple calls to this function will cache and re-use
			/// the same [Metadata].
			...
			/// Check whether this entry is a symbolic link, regardless of whether
				...
				if command.try_arg(&path_to_file).is_err() {
					if self.exec_in_parent_dir {
						match file_info.path().parent() {
							...
							Some(parent) if parent == Path::new("") => {
								// Paths like "foo" have a parent of "".  Avoid
								// chdir("").
							},
							Some(parent) => {
								command.current_dir(parent);
	...
	mod logical_matchers {
		...
		//! This modules contains the matchers used for combining other matchers
		//! and performing boolean logic on them (and a couple of trivial
		//! always-true and always-false matchers). The design is strongly tied
		//! to the precedence rules when parsing command-line options (e.g.
		//! "-foo -o -bar -baz" is equivalent to "-foo -o ( -bar -baz )", not "(
		//! -foo -o -bar ) -baz").
		use std::{error::Error, path::Path};

		use super::{Matcher, MatcherIO, WalkEntry};

		/// This matcher contains a collection of other matchers. A file only
		/// matches if it matches ALL the contained sub-matchers. For
		/// sub-matchers that have side effects, the side effects occur in the
		/// same order as the sub-matchers were pushed into the collection.
		...
		impl Matcher for AndMatcher {
			/// Returns true if all sub-matchers return true. Short-circuiting does
			/// take place. If the nth sub-matcher returns false, then we
			/// immediately return and don't make any further calls.
		...
		/// if it matches any of the contained sub-matchers. For sub-matchers that
		/// have side effects, the side effects occur in the same order as the
		/// sub-matchers were pushed into the collection.
		...
		impl Matcher for OrMatcher {
			/// Returns true if any sub-matcher returns true. Short-circuiting does
			/// take place. If the nth sub-matcher returns true, then we
			/// immediately return and don't make any further calls.
			fn matches(&self, dir_entry: &WalkEntry, matcher_io: &mut MatcherIO) -> bool {
				for matcher in &self.submatchers {
					if matcher.matches(dir_entry, matcher_io) {
		...
		/// regardless of the results of previous submatchers. This is primarily
		/// used for submatchers with side-effects. For such sub-matchers the
		/// side effects occur in the same order as the sub-matchers were pushed
		/// into the collection.
			...
			pub fn check_new_and_condition(&mut self) -> Result<(), Box<dyn Error>> {
				{
					...
					if grandchild_and_matcher.submatchers.is_empty() {
						return Err(From::from(
							"invalid expression; you have used a binary operator '-a' with nothing \
							 before it.",
						));
					}
				}
			...
			pub fn new_list_condition(&mut self) -> Result<(), Box<dyn Error>> {
				{
					...
					if grandchild_and_matcher.submatchers.is_empty() {
						return Err(From::from(
							"invalid expression; you have used a binary operator ',' with nothing before \
							 it.",
						));
					}
				}
	...
	mod perm {
		...
		//! find's permission matching uses a very unix-centric approach, that
		//! would be tricky to both implement and use on a windows platform. So
		//! we don't even try.
	...
	mod printf {
		...
		/// This matcher prints information about its files to stdout, following
		/// GNU find's printf syntax.
		pub struct Printf {
			format:      FormatString,
			output_file: Option<File>,
		}
	...
	mod size {
		...
		/// Matcher that checks whether a file's size if {less than | equal to |
		/// more than} N units in size.
	...
	pub mod time {
		...
		/// This matcher checks whether a file is newer than the file the matcher
		/// is initialized with.
		...
		impl NewerMatcher {
			...
			/// Implementation of matches that returns a result, allowing use to
			/// use try! to deal with the errors.
		...
		impl NewerOptionType {
			#[allow(
				...
				reason = "invalid options default to modified time"
			)]
		...
		/// This matcher checks whether files's accessed|creation|modification
		/// time is newer than the given times.
		...
		/// Provide access to the *change* timestamp, since std::fs::Metadata
		/// doesn't expose it.
		...
		/// This matcher checks whether a file's accessed|creation|modification
		/// time is {less than | exactly | more than} N days old.
	...
	/// is what's being searched for. To a first order approximation, find
	/// consists of building a chain of Matcher objects, and then walking a
	/// directory tree, passing each entry to the chain of Matchers.
	pub trait Matcher: 'static {
		...
		/// command, deleting a file). Iff no such matcher exists in the chain,
		/// then the filename will be printed to stdout. While this is a
		/// compile-time fact for most matchers, it's run-time for matchers that
		/// contain a collection of sub-Matchers.
		fn has_side_effects(&self) -> bool {
	...
	fn convert_arg_to_comparable_value_and_suffix(
		...
		Err(From::from(format!(
			"Expected a decimal integer (with optional + or - prefix) and (optional suffix) argument \
	...
	///
	/// Additionally, there is support for the -anewer and -cnewer short
	...
	/// itself recursively if it encounters an opening bracket. A successful
	/// return consists of a tuple containing the new index into the args array
	/// to use (if called recursively) and the resulting matcher.
...
impl Default for Config {
	fn default() -> Self {
		Self {
			...
			today_start:       false,
			// Directory information and traversal are handled by omp_walker,
...
fn walker_follow_links(follow: Follow) -> omp_walker::FollowLinks {
	match follow {
		Follow::Never => omp_walker::FollowLinks::Never,
		Follow::Roots => omp_walker::FollowLinks::Roots,
		Follow::Always => omp_walker::FollowLinks::Always,
	}
}

fn build_find_walk_request(config: &Config, root: &Path) -> omp_walker::WalkRequest {
	omp_walker::WalkRequest::new(root)
		...
		.detail(omp_walker::WalkDetail::Minimal)
		.order(if config.sorted_output {
			omp_walker::WalkOrder::Path
		} else {
			omp_walker::WalkOrder::Unordered
		})
		.emit_root(true)
		.depth(config.min_depth, config.max_depth)
		.visit_order(if config.depth_first {
			omp_walker::VisitOrder::ContentsFirst
		} else {
			omp_walker::VisitOrder::PreOrder
		})
		.directory_errors(omp_walker::DirectoryErrorMode::Visit)
...
fn process_dir_walk_request(
	...
	let status = request.for_each_entry_with_heartbeat(
		...
		|entry: omp_walker::EntryMeta<'_>| {
			...
			if should_quit {
				local_quit.set(true);
				Ok(omp_walker::WalkDecision::Stop)
			} else if should_skip_current_dir {
				Ok(omp_walker::WalkDecision::SkipDescend)
			} else {
				Ok(omp_walker::WalkDecision::Include)
			}
		},
		|error| {
			ret.set(1);
	...
	match status {
		...
		Err(omp_walker::WalkError::Interrupted(err)) => {
			ret_value = 1;
