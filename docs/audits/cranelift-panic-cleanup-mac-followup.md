...
document records those numbers. It does not make the decision. A companion
measurement of sccache on the same host is in
[`sccache-mac-measurement.md`](sccache-mac-measurement.md).
...
| **Tool** | `scripts/bench-dev-backend.py` and `.github/workflows/bench-dev-backend.yml` at `c57359a77` on `claude/llvm-dev-benchmark`. That branch is experimental; neither file is in `omp2`. 3 repetitions. |
